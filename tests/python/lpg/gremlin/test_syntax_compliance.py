...
def social_graph(db):
    ...
    Topology:
        ...
        Alice -knows(since:2020)-> Charlie
        Bob   -knows(since:2019)-> Charlie
        Bob   -knows(since:2021)-> Diana
        Alice -works_at(role:'engineer')-> Acme
        Bob   -works_at(role:'manager')-> Acme

    Node properties:
        Alice:   Person  age=30, city="NYC",     score=4.5
        Bob:     Person  age=25, city="LA",      score=3.8
    ...
    alice = db.create_node(
        ["Person"], {"name": "Alice", "age": 30, "city": "NYC", "score": 4.5}
    )
    bob = db.create_node(
        ["Person"], {"name": "Bob", "age": 25, "city": "LA", "score": 3.8}
    )
    ...
    e1 = db.create_edge(alice.id, bob.id, "knows", {"since": 2018})
    e2 = db.create_edge(alice.id, charlie.id, "knows", {"since": 2020})
    e3 = db.create_edge(bob.id, charlie.id, "knows", {"since": 2019})
    e4 = db.create_edge(bob.id, diana.id, "knows", {"since": 2021})
    e5 = db.create_edge(alice.id, acme.id, "works_at", {"role": "engineer"})
    e6 = db.create_edge(bob.id, acme.id, "works_at", {"role": "manager"})
...
class TestGremlinSourceSteps:
    ...
    def test_add_e_with_property(self, social_graph):
        ...
        _exec(
            db,
            "g.V().has('name', 'Diana').addE('knows').to(g.V().has('name', 'Alice'))"
            ".property('since', 2025)",
        )
...
class TestGremlinNavigation:
    ...
    def test_out_labeled(self, social_graph):
        ...
        db, _ = social_graph
        rows = _rows(db, "g.V().has('name', 'Alice').out('knows')")
        assert len(rows) == 2  # Bob, Charlie

    def test_out_unlabeled(self, social_graph):
        """out() follows all outgoing edges regardless of label."""
        db, _ = social_graph
        # Alice has knows->Bob, knows->Charlie, works_at->Acme
        rows = _rows(db, "g.V().has('name', 'Alice').out()")
        assert len(rows) == 3

    def test_in_labeled(self, social_graph):
        """in('knows') follows incoming 'knows' edges."""
        db, _ = social_graph
        # Charlie receives knows from Alice and Bob
    ...
    def test_in_unlabeled(self, social_graph):
        ...
        db, _ = social_graph
        # Acme receives works_at from Alice and Bob
    ...
    def test_both(self, social_graph):
        ...
        db, _ = social_graph
        # Bob: in(knows) from Alice, out(knows) to Charlie and Diana
        rows = _rows(db, "g.V().has('name', 'Bob').both('knows')")
        assert len(rows) == 3
    ...
    def test_out_e(self, social_graph):
        ...
        db, _ = social_graph
        rows = _rows(db, "g.V().has('name', 'Alice').outE('knows')")
        assert len(rows) == 2
    ...
    def test_both_e(self, social_graph):
        ...
        db, _ = social_graph
        # Bob: inE(knows)=1 (from Alice), outE(knows)=2 (to Charlie, Diana)
        rows = _rows(db, "g.V().has('name', 'Bob').bothE('knows')")
        assert len(rows) == 3
    ...
    def test_out_v(self, social_graph):
        ...
        db, _ = social_graph
        rows = _rows(db, "g.V().has('name', 'Alice').outE('knows').outV()")
        # Each edge's source is Alice, so 2 results all pointing to Alice
        assert len(rows) == 2
    ...
    def test_other_v(self, social_graph):
        ...
        db, _ = social_graph
        rows = _rows(db, "g.V().has('name', 'Alice').outE('knows').otherV()")
        # From Alice's perspective, otherV is the far end: Bob, Charlie
        assert len(rows) == 2
...
class TestGremlinFilter:
    ...
    def test_has_key_value(self, social_graph):
        ...
        db, _ = social_graph
        rows = _rows(db, "g.V().has('name', 'Alice')")
        assert len(rows) == 1
    ...
    def test_has_id(self, social_graph):
        ...
        db, ids = social_graph
        rows = _rows(db, f"g.V().hasId({ids['bob']})")
        assert len(rows) == 1
    ...
    def test_dedup(self, social_graph):
        ...
        db, _ = social_graph
        # Charlie is reachable from both Alice and Bob, but dedup keeps one
        rows = _rows(db, "g.V().hasLabel('Person').out('knows').dedup()")
        # Alice->Bob,Charlie; Bob->Charlie,Diana; dedup => Bob, Charlie, Diana
        assert len(rows) == 3
...
class TestGremlinMap:
    ...
    def test_id_step(self, social_graph):
        ...
        db, ids = social_graph
        rows = _rows(db, "g.V().has('name', 'Alice').id()")
        assert len(rows) == 1
    ...
    def test_label_step(self, social_graph):
        ...
        db, _ = social_graph
        rows = _rows(db, "g.V().has('name', 'Alice').label()")
        assert len(rows) == 1
    ...
    def test_values_single_key(self, social_graph):
        ...
        db, _ = social_graph
        rows = _rows(db, "g.V().has('name', 'Alice').values('age')")
        assert len(rows) == 1
        val = rows[0] if not isinstance(rows[0], dict) else next(iter(rows[0].values()))
        assert val == 30
    ...
    def test_values_multiple_keys(self, social_graph):
        ...
        db, _ = social_graph
        rows = _rows(db, "g.V().has('name', 'Alice').values('name', 'age')")
        assert len(rows) == 2

    def test_value_map(self, social_graph):
        """valueMap() returns property maps for each vertex."""
        db, _ = social_graph
        rows = _rows(db, "g.V().has('name', 'Alice').valueMap()")
        assert len(rows) >= 1
    ...
    def test_element_map(self, social_graph):
        ...
        db, _ = social_graph
        rows = _rows(db, "g.V().has('name', 'Alice').elementMap()")
        assert len(rows) == 1
        row = rows[0]
        assert isinstance(row, dict)
    ...
    def test_select_after_as(self, social_graph):
        ...
        rows = _rows(
            db,
            "g.V().has('name', 'Alice').as('a').out('knows').select('a')",
        )
    ...
    def test_project(self, social_graph):
        ...
        rows = _rows(
            db,
            "g.V().has('name', 'Alice').project('n', 'a').by('name').by('age')",
        )
        assert len(rows) == 1
        proj = rows[0]
        assert isinstance(proj, dict)
        assert proj.get("n") == "Alice"
    ...
    def test_union(self, social_graph):
        ...
        rows = _rows(
            db,
            "g.V().has('name', 'Alice').union(out('knows'), out('works_at'))",
        )
    ...
    def test_coalesce(self, social_graph):
        ...
        db, _ = social_graph
        # Diana has no 'works_at' but does have incoming 'knows' (from Bob)
        rows = _rows(
            db,
    ...
    def test_order_asc(self, social_graph):
        ...
        assert first == "Bob"  # youngest at 25
...
class TestGremlinSideEffects:
    ...
    def test_drop_edge(self, social_graph):
        ...
        db, _ = social_graph
        before = len(_rows(db, "g.V().has('name', 'Alice').outE('knows')"))
        assert before == 2
        _exec(db, "g.V().has('name', 'Alice').outE('knows').limit(1).drop()")
...
class TestGremlinPredicates:
    ...
    def test_outside(self, social_graph):
        ...
        # age < 26 or age > 34: Bob(25), Charlie(35)
        assert len(rows) == 2
    ...
    def test_predicate_with_navigation(self, social_graph):
        ...
        rows = _rows(
            db,
            "g.V().has('name', 'Alice').outE('knows').has('since', lt(2020)).inV().values('name')",
        )
        # Alice->Bob has since=2018 (< 2020)
        assert len(rows) == 1
        val = rows[0] if isinstance(rows[0], str) else next(iter(rows[0].values()))
        assert val == "Bob"
